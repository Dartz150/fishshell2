...
#ifdef NDS
 #define _VARS_IN_RAM
// #define _VARS_IN_RAM DATA_IN_DTCM
#else
...
static bool FAT_GetFilename (DIR_ENT dirEntry, char* alias);
...
// Long File names
//static char lfnName[MAX_FILENAME_LENGTH];
#include "gba_nds_fat_lfnunicodedef.h"
bool lfnExists;
...
//DATA_IN_DTCM 
char fatBuffer[BYTE_PER_READ];
u32 fatBufferCurSector;

// Current working directory
u32 curWorkDirCluster;

__attribute__ ((section (".dtcm"))) char curWorkDirPath[MAX_FILENAME_LENGTH];
bool curDirChengeFlag;
...
//DATA_IN_DTCM 
u8 globalBuffer[BYTE_PER_READ];

static void ShowAllOpenFiles(void)
{
  for (u32 idx=0;idx<MAX_FILES_OPEN;idx++){
    FAT_FILE *file = &openFiles[idx];
    if(file->inUse==false){
      _consolePrintf("%d: 0x%08x: Closed. [%s]\n",idx,file,file->tmpfna);
      }else{
      _consolePrintf("%d: 0x%08x: Opened. [%s]\n",idx,file,file->tmpfna);
    }
  }
}
// -----------------------------------------------------------------
...
u32 GetFisrtCls()
{
    return filesysData - 2*filesysSecPerClus;
}
...
u32 FAT_GetOpenedFileHandlesCount(void)
{
  u32 cnt=0;
  for(u32 idx=0;idx<MAX_FILES_OPEN;idx++){
    if(openFiles[idx].inUse==true) cnt++;
  }
  ShowAllOpenFiles();
  return(cnt);
}
  ...
  switch (filesysType) 
  {
    ...
    case FS_FAT12:
      ...
      if (sector != fatBufferCurSector)
      {
        ...
        disc_SystemReadSector(fatBufferCurSector, fatBuffer);
      }
    ...
    case FS_FAT12:
      ...
      if (cluster & 0x01) {
        ...
        if (offset >= BYTE_PER_READ) {
          ...
          disc_SystemWriteSector(fatBufferCurSector, fatBuffer);
...
u32 FAT_FirstFreeCluster(void)
{
  ...
  if(fatFirstFree==CLUSTER_FREE) fatFirstFree = CLUSTER_FIRST;
  if(fatFirstFree<CLUSTER_FIRST) fatFirstFree = CLUSTER_FIRST;
  
  for(u32 idx=fatFirstFree;idx<=fatLastCluster;idx++){
    if(FAT_NextCluster(idx)==CLUSTER_FREE){
      fatFirstFree=idx;
      return(idx);
    }
  }
  for(u32 idx=CLUSTER_FIRST;idx<fatFirstFree;idx++){
    if(FAT_NextCluster(idx)==CLUSTER_FREE){
      fatFirstFree=idx;
      return(idx);
    }
...
bool FAT_InitFiles (void)
{
  ...
  else  // This is an MBR
  {
    ...
    _consolePrintf("MBR boot sector=%d ",bootSector);
  }

  // Read in boot sector
  bootSec = (BOOT_SEC*) globalBuffer;
  disc_SystemReadSector (bootSector,  bootSec);
  ...
  _consolePrintf("RD=%d RE=%d REsize=%d\n",filesysRootDir,bootSec->rootEntries,sizeof(DIR_ENT));
  _consolePrintf("SPC=%d FD=%d FLC=%d\n",bootSec->sectorsPerCluster,filesysData,fatLastCluster);

  if (fatLastCluster < 4085)
  {
    _consolePrint("Detect FAT12\n");
    filesysType = FS_FAT12;  // FAT12 volume - unsupported
  }
  else if (fatLastCluster < 65525)
  {
    _consolePrint("Detect FAT16\n");
    filesysType = FS_FAT16;  // FAT16 volume
  }
  else
  {
    _consolePrint("Detect FAT32\n");
  ...
  if (filesysType != FS_FAT32)
  {
    ...
    for (i = 0; i < 11; i++) {
        filesysVolumeLabel[i] = bootSec->extBlock.fat16.volumeLabel[i];
    }
  }
  else  // Set up for the FAT32 way
  {
    ...
    for (i = 0; i < 11; i++) {
        filesysVolumeLabel[i] = bootSec->extBlock.fat32.volumeLabel[i];
    }
  }
  ...
  {
//    lfnName[i] = '\0';
#include "gba_nds_fat_initfiles1.h"
  }
...
DIR_ENT FAT_GetDirEntry ( u32 dirCluster, int entry, int origin)
{
  ...
  do {
    ...
    if ((dir.name[0] != FILE_FREE) && ((dir.name[0] > 0x20) || (dir.name[0] == 0x05)) && ((dir.attrib & ATTRIB_VOL) != ATTRIB_VOL))
    {
      entry--;
      if (lfnExists)
      {
        ...
        chkSum = 0;
        for (aliasPos=0; aliasPos < 8; aliasPos++)
        {
          // NOTE: The operation is an unsigned char rotate right
          chkSum = ((chkSum & 1) ? 0x80 : 0) + (chkSum >> 1) + dir.name[aliasPos];
        }
        for (aliasPos=0; aliasPos < 3; aliasPos++)
        {
          // NOTE: The operation is an unsigned char rotate right
        ...
        if (chkSum != lfnChkSum)
        {
          lfnExists = false;
//          lfnName[0] = '\0';
#include "gba_nds_fat_getdirentry1.h"
        }
      }
      if (entry == 0) 
      {
        ...
        {
          char alias[14];
          FAT_GetFilename (dir, alias);
#include "gba_nds_fat_getdirentry2.h"
        }
        found = true;
      }
    }
    ...
    {
      DIR_ENT_LFN lfn = *((DIR_ENT_LFN*)&dir);
...
#include "gba_nds_fat_getdirentry3.h"
...
#include "gba_nds_fat_getdirentry4.h"
      }
    }
...
#include "gba_nds_fat_GetAllDirEntry.h"
...
DIR_ENT FAT_DirEntFromPath (const char* path)
{
  ...
  while (!notFound && !found)
  {
    ...
    if(flagLFN==true){
      ...
      while(*path!=0){
        _consolePrintf("0x%02x,",*path);
        path++;
      }
      _consolePrint("\n");

      StopFatalError(11901,"FAT_DirEntFromPath: Not support find from LFN path.\n");
    }
...
*/
    ...
    if (curDirChengeFlag && found && ((dirEntry.attrib & ATTRIB_DIR) == ATTRIB_DIR))
    {
        if(alias[0]=='.' && alias[1]=='\0'){
        }else if(alias[0]=='.' && alias[1]=='.' && alias[2]=='\0'){
            char *ptag=curWorkDirPath;
            u32 slashidx=0;
            u32 idx=0;
            while(ptag[idx]!='\0'){
                if(ptag[idx]=='/') slashidx=idx;
                idx++;
            }
            if(slashidx==0){
                ptag[0]='/';
            ...
            char *ptag=curWorkDirPath;
            if(dirCluster==filesysRootDirClus) {
                ptag[0]='/';
                ptag[1]='\0';
            }
            if(ptag[1]!='\0'){
...
#include "gba_nds_fat_DirEntFromPath_LFNTopDirEntry.h"
...
bool FAT_AddDirEntry (const char* path, DIR_ENT newDirEntry)
{
  ...
  while (path[pathPos + filePos] != '\0')
  {
    if (path[pathPos + filePos] == '/')
    {
      ...
      if (FAT_chdir(filename) == false) 
      {
        ...
        _consolePrint("Can't change directory.\n");
  ...
  while ((filePos < MAX_FILENAME_LENGTH - 1) && (path[pathPos] != '\0'))
  {
    ...
    {
      flagLFN = true;
      _consolePrint("Add entry: Invalid character.\n");
    }
    if (filename[filePos] == '.') {
      if (!dotSeen) {
        dotSeen = true;
        ...
        flagLFN = true;
        _consolePrint("Add entry: Multi dot?\n");
      }
    }
    filePos++;
    pathPos++;
    if ((filePos > 8) && !dotSeen) {
      flagLFN = true;
      _consolePrint("Add entry: Long filename?\n");
    }
  }
  filename[filePos] = 0; // This set terminate? for get extention length.
  ...
  if (!flagLFN && (strrchr (filename, '.') != NULL) && (strlen(strrchr(filename, '.')) > 4)) {
    flagLFN = true;
    _consolePrint("Add entry: Long extention.\n");
  }
  ...
  if(flagLFN==true){
    _consolePrint("Add entry: Can not create long filename file.\n");
//    while(1);
    flagLFN=false;
  }
  ...
  _consolePrintf("AddEntry: Point info. dirCluster=%d, secOffset=%d, entryOffset=%d.\n",dirCluster,secOffset,entryOffset);
...
static char FindFileResultBuffer[MAX_FILENAME_LENGTH];
...
FILE_TYPE FAT_FindNextFile(const char** filename)
{
  ...
  // Get the filename
  FindFileResultBuffer[0]=0;
  FAT_GetFilename (file, FindFileResultBuffer);
  *filename=FindFileResultBuffer;
...
bool FAT_chdir (const char* path)
{
  ...
  //Refresh directory path flag
  curDirChengeFlag=true;
...
FAT_FILE* FAT_fopen(const char* path, const char* mode)
{
  ...
  if (pchTemp == NULL) StopFatalError(11902,"Unknown file open mode error.\n");
  if (strpbrk ( pchTemp+1, "rRwWaA" ) != NULL) StopFatalError(11903,"Duplicated file open mode error.\n");
  
  // Get the dirEntry for the path specified
  dirEntry = FAT_DirEntFromPath (path);
  
  // Check that it is not a directory
  if (dirEntry.attrib & ATTRIB_DIR) StopFatalError(11904,"Can not open directory.\n");

#ifdef CAN_WRITE_TO_DISC
  // Check that it is not a read only file being openned in a writing mode
  if ( (strpbrk(mode, "wWaA+") != NULL) && (dirEntry.attrib & ATTRIB_RO)) StopFatalError(11905,"Can not open ReadOnly file for write.\n");
#else
  if ( (strpbrk(mode, "wWaA+") != NULL)) StopFatalError(11906,"Not impliments file write.\n");
#endif

  // Find a free file buffer
  for (fileNum = 0; (fileNum < MAX_FILES_OPEN) && (openFiles[fileNum].inUse == true); fileNum++);
  
  if (fileNum == MAX_FILES_OPEN){
    ShowAllOpenFiles();
    StopFatalError(11907,"FAT_FILE buffer full.\n"); // No free files
  }
  ...
  if ( strpbrk(mode, "rR") != NULL )  //(ucase(mode[0]) == 'R')
  {
    ...
    if (file->write && file->firstCluster == CLUSTER_FREE)
    {
      ...
      {
        StopFatalError(11908,"Create first cluster error on read/write mode. Disk full!!\n");
      }
  ...
  if ( strpbrk(mode, "wW") != NULL ) // (ucase(mode[0]) == 'W')
  {
    ...
    {
      StopFatalError(11909,"Create first cluster error on write mode. Disk full!!\n");
    }
    ...
    if (dirEntry.name[0] == FILE_FREE)  // No file
    {
      ...
      if(!FAT_AddDirEntry (path, dirEntry)) StopFatalError(11910,"Can not add dir entry.\n");
  ...
  if ( strpbrk(mode, "aA") != NULL ) // (ucase(mode[0]) == 'A')
  {
    if (dirEntry.name[0] == FILE_FREE)  // Create file if it doesn't exist
    {
      ...
      if (startCluster == CLUSTER_FREE)  // Couldn't get a free cluster
      {
        StopFatalError(11911,"Create first cluster error on append mode. Disk full!!\n");
      }
      dirEntry.startCluster = (startCluster & 0xFFFF);
      dirEntry.startClusterHigh = ((startCluster >> 16) & 0xFFFF);
      
      if(!FAT_AddDirEntry (path, dirEntry)) StopFatalError(11912,"Can not add dir entry.\n");
    ...
    else  // File already exists - reuse the old directory entry
    {
      ...
      if (startCluster == CLUSTER_FREE)
      {
        ...
        if (file->firstCluster == CLUSTER_FREE)  // Couldn't get a free cluster
        {
          StopFatalError(11911,"Create first cluster error on exists write mode. Disk full!!\n");
        }
...
bool FAT_fclose (FAT_FILE* file)
{
  ...
  if ((file != NULL) && (file->inUse == true))
  {
    ...
    if (file->write || file->append)
    {
      if(file->length==0){
        ShowAllOpenFiles();
        StopFatalError(11915,"0x%08x: Not support 0byte file writed.\n",file);
      }
      ...
      ((DIR_ENT*)globalBuffer)[file->dirEntOffset].mTime = SystemDateTime_Time;
      ((DIR_ENT*)globalBuffer)[file->dirEntOffset].mDate = SystemDateTime_Date;
      ((DIR_ENT*)globalBuffer)[file->dirEntOffset].aDate = SystemDateTime_Date;
...
u32 FAT_ftell (FAT_FILE* file)
{
  ...
  else
  {
    ...
    ShowAllOpenFiles();
    StopFatalError(11916,"0x%08x: Can not tell. not opened.\n",file);
    return(-1);
  }
}
...
int FAT_fseek(FAT_FILE* file, s32 offset, int origin)
{
  ...
  {
    ShowAllOpenFiles();
    StopFatalError(11917,"0x%08x: Can not seek. not opened.\n",file);
  }
  ...
  {
    ShowAllOpenFiles();
    StopFatalError(11918,"0x%08x: Can not seek in modify mode.\n",file);
  }
  ...
  default:
    ShowAllOpenFiles();
    StopFatalError(11919,"0x%08x: Can not seek. unknown origin mode.\n",file);
  }
...
#include "gba_nds_fat_fread.h"
#include "gba_nds_fat_fread_fast.h"
#include "gba_nds_fat_fskip.h"
...
u32 FAT_fwrite (const void* buffer, u32 size, u32 count, FAT_FILE* file)
{
  ...
  if ((file == NULL) || (file->inUse == false)){
    ShowAllOpenFiles();
    StopFatalError(11920,"0x%08x: Can not write. not opened.\n",file);
  }
  ...
  {
    ShowAllOpenFiles();
    StopFatalError(11921,"0x%08x: Can not write. opened for read.\n",file);
  }
  ...
  if (curSect >= filesysSecPerClus)
  {
    ...
    if ((tempNextCluster == CLUSTER_EOF) || (tempNextCluster == CLUSTER_FREE))
    {
      ...
      MemSet32CPU(0, writeBuffer, BYTE_PER_READ);
    } else {
  ...
  {
    MemCopy8CPU(data,&(writeBuffer[curByte]), tempVar);
    remain -= tempVar;
    data += tempVar;
    curByte += tempVar;
  ...
  if ( (( (file->append ? file->appByte : file->curByte) + length) >= BYTE_PER_READ) && flagNoError)
  {
    ...
    if (remain > 0) {
      MemCopy8CPU(data,writeBuffer, remain);
      curByte += remain;
      remain = 0;
      disc_WriteSector( curSect + FAT_ClustToSect( curClus), writeBuffer);
    }
...
int FAT_remove (const char* path)
{
  ...
  if (dirEntry.name[0] == FILE_FREE) StopFatalError(11922,"Can not remove. This is removed.\n");
  ...
  if (dirEntry.attrib & ATTRIB_DIR)
  {
    ...
    if (checkFiletype != FT_NONE)
    {
      ...
      ShowAllOpenFiles();
      StopFatalError(11923,"Can not tell. Directory isn't empty.\n");
    }
  }
...
int FAT_mkdir (const char* path)
{
  ...
  if (FAT_FileExists(path) != FT_NONE) StopFatalError(11924,"Can not mkdir. Directory is exists.\n");
  ...
  while (path[pathPos + filePos] != '\0')
  {
    if (path[pathPos + filePos] == '/')
    {
      ...
      if (FAT_chdir(pathname) == false) 
      {
        ...
        StopFatalError(11925,"Can not mkdir. Can not chdir directory.\n");
      }
      pathPos += filePos + 1;
      filePos = 0;
    }
  ...
  if (newDirCluster == CLUSTER_FREE) StopFatalError(11926,"Can not tell. Couldn't get a new cluster for the directory.\n");
  ...
  if (FAT_AddDirEntry (path, dirEntry) == false) StopFatalError(11927,"Can not tell. Couldn't add the directory entry.\n");

  // Create the new directory itself
  MemSet32CPU(FILE_LAST, entries, BYTE_PER_READ); // FILE_LAST == 0
  ...
  MemCopy32CPU(&dirEntry, entries, sizeof(dirEntry));
  ...
  MemCopy32CPU(&dirEntry, &entries[1], sizeof(dirEntry));

  // Write entry to disc
  disc_SystemWriteSector(FAT_ClustToSect(newDirCluster), entries);
...
void FAT_freopen(FAT_FILE* file)
{
  ...
  if ((file != NULL) && (file->inUse == true))
  {
    ...
    if (file->write || file->append)
    {
      if(file->length==0){
        ShowAllOpenFiles();
        StopFatalError(11928,"0x%08x: Not support 0byte file writed.\n",file);
      }
...
#include "gba_nds_fat_move.h"
#include "gba_nds_fat_deletefile.h"
void FAT_ShowDirectoryEntryList(const char *pPath)
{
  if(FAT_chdir(pPath)==false){
    _consolePrintf("Can not chdir. [%s]\n",pPath);
    while(1);
  }
  
  _consolePrintf("Start entry list. [%s]\n",pPath);
  ...
  while (1) {
    if(dirEntry==NULL){ // Couldn't find specified file
      _consolePrintf("End entry list.\n");
      return;
    }
    
    // Check against alias as well.
    if(dirEntry->name[0] == FILE_FREE){
      _consolePrintf("%d: Free entry.\n",CurEntryIndex);
      }else{
      if(dirEntry->name[0] == FILE_LAST){
        // _consolePrintf("%d: Last entry.\n",CurEntryIndex);
        }else{
        if((dirEntry->attrib & ATTRIB_LFN) == ATTRIB_LFN){
          _consolePrintf("%d: LFN entry. attrib= 0x%02x\n",CurEntryIndex,dirEntry->attrib);
          }else{
          if((dirEntry->name[0] <= 0x20) && (dirEntry->name[0] != 0x05)){
            _consolePrintf("%d: Invalid entry. attrib= 0x%02x\n",CurEntryIndex,dirEntry->attrib);
            }else{
            char alias[13];
            FAT_GetFilename(*dirEntry, alias);
            _consolePrintf("%d: Alias entry. attrib= 0x%02x [%.13s]\n",CurEntryIndex,dirEntry->attrib,alias);
          }
        }
      }
    }
...
/*
    u8 *pu8=(u8*)dirEntry;
    for(u32 idx=0;idx<sizeof(DIR_ENT);idx++){
      _consolePrintf("%02x,",pu8[idx]);
    }
    _consolePrintf("\n");
*/
